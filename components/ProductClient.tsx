...
// 1. Define the interface representing the product structure
interface Product {
  id: string | number;
  name: string;
  brand: string;
...
// 2. Define the props structure for the client view component
interface ProductClientViewProps {
  product: Product;
}

export default function ProductClientView({ product }: ProductClientViewProps) {
  // Initialize states with implicit/explicit primitive types cleanly
  const [activeImage, setActiveImage] = useState<string>(product.images?.[0] || "");
  const [quantity, setQuantity] = useState<number>(1);
  const [activeTab, setActiveTab] = useState<string>("description");

  // Typed parameter explicitly to prevent structural implicit 'any' compile checks
            ...
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <button 
                  ...
                  className="flex-1 bg-[#3b82f6] text-white font-bold py-3 px-8 rounded-md hover:bg-blue-700 active:scale-[0.99] transition uppercase tracking-wider text-xs shadow-md shadow-blue-100"
                >
                  Add To Cart
                </button>
